...
<!-- 비밀번호-->
<c:if test="${cnfVO.pw_use_fg eq 'Y' && dataVO.idTbl_data eq null}">
	<tr>
			<th>
				비밀번호				
			</th>
...
<!-- 비밀글 -->
<c:if test="${cnfVO.lock_use_fg eq 'Y' && dataVO.idTbl_data eq null}">
	<tr>
			<th>
				비밀글 여부				
			</th>
			<td>
				<div class="input-group rows"> 
					<input checked name="isLocked" type="checkbox" value="Y" ${dataVO.isLocked eq 'Y' ? 'checked' :'' } id="isLockedChkbox"/>
					&nbsp; <label for="isLockedChkbox">비밀글이며 관리자만 읽을 수 있습니다.</label>
				</div> 
			</td>
	</tr>
</c:if>
