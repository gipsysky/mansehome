...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>  
      ...
      <div class="table-responsive">
        <table class="table table-striped table-sm border-bottom ">
          ...
          <tbody> 
			<c:forEach items="${vendor }" var="list" varStatus="status">
	            <tr>
	              <td><c:out value="${ (pageMaker.total - status.index) - (pageMaker.cri.pageNum - 1) * 10 }"/></td>
	              <td><a href="javascript:move('${list.idVendor }');">${list.comp_name }</a></td>
	              <td>${list.codesVO.name }</td> 
	              <td>${list.ven_id}</td>
	              <td>${list.comp_ceo_name}</td> 
	              <td>
	              	${list.ven_status eq "Y" ? "진행중":""}
	              	${list.ven_status eq "N" ? "중지":""}
	              	${list.ven_status eq "I" ? "승인전":""}
	              </td> 
	            </tr>
	        </c:forEach>  
          </tbody>
		 ...
		 <div class="input-group col-md-6" style="float:none; margin:0 auto;">   
		  ...
		  <div class="input-group-prepend">
		    <select id="sType3" class="form-control">
		    		...
		    		<option value="Y" ${ "Y" eq pageMaker.cri.sType3 ? "selected":"" }>진행중</option>
		    		<option value="N" ${ "N" eq pageMaker.cri.sType3 ? "selected":"" }>중지</option>
		    		<option value="I" ${ "I" eq pageMaker.cri.sType3 ? "selected":"" }>승인전</option>
		    </select>
		  </div>   
		  <div class="input-group-prepend">
		    <select id="sType" class="form-control">
					<option value="" <c:out value="${pageMaker.cri.sType == null?'selected':'' }"/>>선택</option>
					<option value="T" <c:out value="${pageMaker.cri.sType eq 'T'?'selected':'' }"/>>회사명</option>
					<option value="I" <c:out value="${pageMaker.cri.sType eq 'I'?'selected':'' }"/>>아이디</option>  
		    </select>
		  </div> 
