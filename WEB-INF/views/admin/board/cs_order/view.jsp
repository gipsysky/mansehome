...
<c:set var="idOrder_info" value="0"/>
<c:set var="idOrder_products" value="0"/>
			...
			<div class="col-sm-8">
				...
				<c:choose>				
					...
					<c:when test="${list.fld_type eq 'file' and dataVO[fldcode] ne '' }">
						<a href="${dataVO[fldcode] }">${dataVO[fldcode] }</a>
					</c:when>
					<c:otherwise>
						<c:choose>
							<c:when test="${list.fld_code eq '4'}">
								<!-- 주문번호 -->
								<c:set var="idOrder_info" value="${dataVO[fldcode]}"/>
								<a href="/admin/sale/order/view?idOrder_info=${dataVO[fldcode]}">
										${fn:replace(dataVO[fldcode], BR, "<br>")}
								</a>
							</c:when>
							<c:when test="${list.fld_code eq '2'}">
								<!-- 선택한 상품 -->
								<c:set var="idOrder_products" value="${dataVO[fldcode]}"/>
...
//*******************************************************************
$(function(){
	show_selected_product_list();
});
function show_selected_product_list(){
	$.get('/admin/sale/order/view/requestProdList?idOrder_info=<c:out value="${idOrder_info}"/>&idOrder_products=<c:out value="${idOrder_products}"/>', function(r){
		$("#selected_product_list").html(r);
	});
}
//*******************************************************************
//선택한 상품 보기 끝
...
function commentOk(){
	...
	f.action="/web/board/data/commentOk";
